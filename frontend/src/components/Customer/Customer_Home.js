    ...
    return !loading &&  (
        <Fragment>
            <div className="card">
                ...
                (!auth.loading && auth.isAuthenticated && product.name.includes(name) && product.orders.findIndex(x => x.customer === auth.user._id)===-1 && product.state==="waiting") && <ProductButtonElement name={product.name} vendor={product.vendor} quantity={product.quantity} price={product.price} key={product._id} id={product._id} vendorName={product.vendorName} vendorRating={product.vendorRating} />
                ))}
                </div>
            </div>
        </Fragment>
